...
object Utilities {
  ...
  def maybeTransposeInputSequences[T](sequences: Sequences[T])(implicit context: Context): Sequences[T] = {
    if (context.modelConfig.timeMajor)
      sequences.copy(sequences = sequences.sequences.transpose(Seq(1, 0, 2)))
    else
      sequences
  }
}
