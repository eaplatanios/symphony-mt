...
) extends Metric[((Output, Output), ((Output, Output), (Output, Output, Output))), Output] {
  override def compute(
      values: ((Output, Output), ((Output, Output), (Output, Output, Output))),
      weights: Output = null,
      name: String = name
  ): Output = {
    tf.createWithNameScope(name) {
      ...
      val loss = tf.sum(tf.sequenceLoss(
        values._1._1, values._2._2._2, mask, averageAcrossTimeSteps = false, averageAcrossBatch = false))
